// GESTAOMAX COMERCIAL V1 - LICENCA + MULTI-EMPRESA
...
function validarChave(chave){
  ...
  let expStr = partes[partes.length-2]; // AAAAMMDD
...
function telaLicenca(motivo=''){
  ...
  <div style="background:white;border-radius:20px;max-width:400px;width:100%;padding:25px;text-align:center">
    <h2 style="color:#0d47a1">GESTAOMAX</h2>
    <p style="color:#666">${motivo || 'Sistema Licenciado'}</p>
    <p style="font-size:13px;background:#fff3e0;padding:10px;border-radius:8px">Para ativar, faça pagamento<br><b>M-Pesa: 84xxxxxxx</b><br>e insira a chave abaixo</p>
    <input id="inputChave" placeholder="GMAX-..." style="width:100%;padding:14px;border:2px solid #ddd;border-radius:12px;margin-top:10px">
    ...
    <p style="font-size:11px;margin-top:15px">Suporte: WhatsApp 84xxxxxxx</p>
  </div></div>`;
}
...
// --- COFRE POR EMPRESA (fim da mistura) ---
...
function setFuncionariosEmpresa(arr){
  ...
  localStorage.setItem('funcionariosRH', JSON.stringify(unicos)); // compat
...
(function(){
  ...
  if(!lic.ok){
    if(lic.expirada) telaLicenca('Licença expirada em '+lic.data.toLocaleDateString()+'<br>Renove via M-Pesa');
    else telaLicenca();
    return;
  }
  // se tem licenca, carrega funcionarios daquela empresa
  window.funcionarios = getFuncionariosEmpresa();
  console.log('EMPRESA:', getEmpresaAtual(), 'FUNC:', window.funcionarios.length);
  // mostra empresa no titulo se existir
  setTimeout(()=>{
...
// --- Tua funcao Ver Minha Situacao definitiva ---
